...
def init_read(
    batch_size, diag_flag, proc_flag, out_flag, chart_flag, med_flag, lab_flag
):
    condVocabDict = {}
    procVocabDict = {}
    medVocabDict = {}
...
def get_batch_data(
    key, data, diag_flag, proc_flag, out_flag, chart_flag, med_flag, lab_flag
):
    conds = []
    procs = []
    meds = []
        ...
        for hid, hid_data in batchChartDict.items():
            ...
            for feature, feat_data in hid_data.items():
                ...
                if feature == "Chart":
                    ...
                    fids = list(
                        map(chartVocabDict.get, list(feat_data["signal"].keys()))
                    )
